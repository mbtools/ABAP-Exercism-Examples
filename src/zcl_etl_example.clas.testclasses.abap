CLASS ltcl_etl DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_etl_example.
    METHODS:
      setup,
...
CLASS ltcl_etl IMPLEMENTATION.
  METHOD setup.
    cut = NEW zcl_etl_example( ).
  ENDMETHOD.

  METHOD single_letter.
    DATA(inp) = VALUE zcl_etl_example=>tty_legacy_data(
                  ( number = 1  string = `A` ) ).

    cl_abap_unit_assert=>assert_equals(
      act = cut->transform( inp )
      exp = VALUE zcl_etl_example=>tty_new_data(
              ( letter = 'a' number = 1 ) ) ).
  ENDMETHOD.
  ...
  METHOD single_score_multi_letter.
    ...
    DATA(exp) = VALUE zcl_etl_example=>tty_new_data(
                  ( letter = 'a' number = 1 )
                  ( letter = 'e' number = 1 )
                  ( letter = 'i' number = 1 )
                  ( letter = 'o' number = 1 )
