...
CLASS ltcl_binary_search DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_binary_search_example.
    METHODS setup.
    METHODS:
...
CLASS ltcl_binary_search IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_binary_search_example( ).
  ENDMETHOD.

  METHOD finds_valid_value.
    DATA(table) = VALUE zcl_binary_search_example=>integertab( ( 6 ) ).
  ...
  METHOD finds_value_in_middle_of_table.
    DATA(table) = VALUE zcl_binary_search_example=>integertab( ( 1 ) ( 3 ) ( 4 ) ( 6 ) ( 8 ) ( 9 ) ( 11 ) ).
    cl_abap_unit_assert=>assert_equals(
      act = cut->binary_search( val = 6 table = table )
      exp = 4 ).
  ENDMETHOD.
  ...
  METHOD finds_value_in_odd_table.
    DATA(table) = VALUE zcl_binary_search_example=>integertab(
  ...
  METHOD not_found_when_bounds_cross.
    DATA(table) = VALUE zcl_binary_search_example=>integertab( ( 1 ) ( 2 ) ).
    TRY.
        cut->binary_search(
          val = 0
          table = table ).
