CLASS ltcl_itab_nesting DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_itab_nesting_example.
    METHODS setup.
...
CLASS ltcl_itab_nesting IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_itab_nesting_example( ).
  ENDMETHOD.
  ...
  METHOD test_empty_input.
    cl_abap_unit_assert=>assert_equals(
      act = cut->perform_nesting( artists = VALUE #( )
                                  albums = VALUE #( )
                                  songs = VALUE #( ) )
      exp = VALUE zcl_itab_nesting_example=>nested_data( ) ).
  ENDMETHOD.
  ...
  METHOD test_single_artist.
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_itab_nesting_example=>nested_data( ( artist_id = '1' artist_name = 'Godsmack'
