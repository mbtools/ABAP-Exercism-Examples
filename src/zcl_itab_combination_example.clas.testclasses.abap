CLASS ltcl_itab_combination DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_itab_combination_example.
    METHODS setup.
...
CLASS ltcl_itab_combination IMPLEMENTATION.
  ...
  METHOD setup.
    cut = NEW zcl_itab_combination_example( ).
  ENDMETHOD.
  ...
  METHOD test_empty_input.
    cl_abap_unit_assert=>assert_equals(
      act = cut->perform_combination( alphas = VALUE #( )
                                      nums = VALUE #( ) )
      exp = VALUE zcl_itab_combination_example=>combined_data( ) ).
  ENDMETHOD.
  ...
  METHOD test_single_row.
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_itab_combination_example=>combined_data( ( colx = 'A1' coly = 'B2' colz = 'C3' ) ) ).
